...
通过数组里面的`parentId`生成树形结构

```js
const getTree = (list, parentId) => {
    const pl = list.filter((item) => item.parentId === parentId);
    pl.forEach((item) => {
        item.childList = getTree(list, item.id);
    });
    return pl;
};
...
// 测试
const list= [
  {name:'父级',id:1,parentId:-1},
  {name:'父级2',id:2,parentId:-1},
  {name:'子级',id:3,parentId:1},
  {name:'子级2',id:4,parentId:2},
]
getTree(list,-1)
//输出
[
  {name:'父级',id:1,parentId:-1,childList:[{name:'子级',id:3,parentId:1,childList:[]}]},
  {name:'父级2',id:2,parentId:-1,childList:[{name:'子级2',id:4,parentId:2,childList:[]}]},
]
```
